...
## 10. Open items to close before demo day (updated Sept 3 2026 — see Sections 11/12 for full detail)
...
All 8 backend must-haves from Section 5 are built. Most were verified end-to-end against a live server with real Razorpay test-mode API calls (confirmed: real orders created, e.g. `order_TXEo3YjftQcKwq`) — but **the table below previously marked `catalog.py`'s freshness check as "verified" when it was actually dead code, never called from `main.py`.** A research-PDF audit caught this by grepping for `check_freshness` and finding only its own definition. That was a real gap between what was claimed and what was true, not a documentation nitpick — corrected below, and the fix is now written (pending the user's own live re-test, see the note under the table).
...
| `firewall/catalog.py` | Content hashing (price+SKU+qty+normalized text) for browse-time vs. checkout-time freshness comparison | Hash logic (`compute_content_hash`) verified standalone. `check_freshness()` was **built but not wired into any endpoint** until this fix — see below. Not yet re-tested live post-fix. |
...
| `firewall/main.py` | FastAPI app: CORE gate (`/intent`, `/catalog/snapshot`, `/checkout`) + EXTENSION (`/payments/capture` with atomic race guard + refund fallback) + `/audit` | 5 scenarios (clean-ALLOW, real-attack-BLOCK, price-hard-block, unknown-order 404, audit readback) passed live **before this fix**. `/checkout` now also calls `check_freshness()` on every request and upgrades ALLOW→REVALIDATE on catalog drift (never downgrades a BLOCK) — written, syntax-checked, **not yet re-run live**. |
