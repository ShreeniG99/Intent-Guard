...
| `firewall/main.py` | FastAPI app: CORE gate (`/intent`, `/catalog/snapshot`, `/checkout`) + EXTENSION (`/payments/capture` with atomic race guard + refund fallback) + `/audit` | Yes — all 6 `test_main.py` scenarios pass live (2026-09-04) on the current code, including `/checkout` calling `check_freshness()` and upgrading ALLOW→REVALIDATE on catalog drift without consuming the token. |
